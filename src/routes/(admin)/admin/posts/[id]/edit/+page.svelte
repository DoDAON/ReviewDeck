<script lang="ts">
	...
	let selectedTagIds: string[] = data.review.tags.map(tag => tag.id);
...
<svelte:head>
	<title>게시물 수정: {data.review.title} - ReviewDeck</title>
</svelte:head>
...
<div class="space-y-6">
	...
	<div class="flex items-center space-x-4">
		<a
			href="/admin/posts/{data.review.id}"
			class="text-gray-500 hover:text-gray-700 transition-colors"
			aria-label="게시물 상세로 돌아가기"
		>
		...
		<div>
			...
			<p class="text-gray-600 mt-1">게시물 정보를 수정하세요.</p>
		</div>
	</div>
			...
			<div>
				...
				>
					{#each ratingOptions as rating}
						<option value={rating} selected={data.review.rating === rating}>
							{rating}
						</option>
					{/each}
				</select>
